...
- **Batch 5 (async ingestion — persist-first, executor, sweep, reaper, orphan fallback): code complete, 39/39 local tests green. NOT YET CI-verified.**
  ...
  - **6 required integration tests written** (all need CI/Docker to execute): `IngestionClaimConcurrencyIT` (20 concurrent threads, asserts sum of claimed rows == 1), `TicketIngestionAfterCommitIT` (polls from a separate JDBC connection), `IngestionRejectionIT` (real `core=1/max=1/queue=1` executor + `CountDownLatch`-blocked task, proves 202+PENDING+later-DONE on genuine saturation), `IngestionReaperIT` (5 cases: stale-reset, fresh-untouched, PENDING/DONE-untouched, null-claimedAt-untouched, exhausted→fallback-team), `TicketIdempotencyConcurrencyIT` (10 concurrent POSTs, same externalRef, asserts exactly 1 row + no 500s).
  - Added `awaitility` test dependency for polling assertions.
- Mandatory pause points (user-specified, still in force): ✅ V1–V4/entities batch — reviewed and passed. ✅ Batch 4 (team-isolation/security) — CI-verified, closed. **⏸ Batch 5 (ingestion durability) — code complete, awaiting CI verification. Per user instruction, do not start Batch 6 until CI is green on Batch 5.**
